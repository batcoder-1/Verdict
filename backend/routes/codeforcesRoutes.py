...
async def sync_codeforces_contest(token:Annotated[str,Depends(oauth2_scheme)],session:SessionDep):
    contests=await sync_contest(token,session)
    return contests

@codeforces.get('/cp_analyzer/profile/codeforces/contest',
                response_model=list[codeforcesContest])
async def get_codeforces_contest(token:Annotated[str,Depends(oauth2_scheme)],session:SessionDep):
